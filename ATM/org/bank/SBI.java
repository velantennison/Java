...
public class SBI implements Bank 
{
	...
	public Account getAccountDetail(long accountNumber) 
	{
		...
		try {
			accountDetail = database.getAccountDetails(accountNumber,"SBI");
	...
	{
		Database database = Database.getDatabase();
		JSONObject cardDetail = database.getCardDetails(cardNumber,"SBI");
		Card card = new Card();
	 ...
	 */
	public JSONObject depositProcess(double amount, Account account) 
	{
		JSONObject transaction = account.updateDepositRecord(amount, account);
		if(transaction != null) 
		{
			System.out.println("\nAmount Deposited \nRs:"+ amount);
			return transaction;
	 ...
	 * @return transaction detail
	 */
	public JSONObject withdrawalProcess(double amount, Account account) 
	{
		JSONObject transaction = account.updateWithdrawalRecord(amount, account);
		if(transaction != null) 
		{
			System.out.println("\nAmount Withdrew \nRs:"+ amount);
			return transaction;
		...
		{
			withdrawalProcess(amount,account);
